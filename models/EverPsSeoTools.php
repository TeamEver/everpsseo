...
class EverPsSeoTools extends ObjectModel
{
    ...
    */
    ...
    public static function updateDatabaseMediasToWebP($objName, $objId, $content, $idLang, $contentType = 'description')
    {
        ...
        switch ($objName) {
            case 'product':
                $ps_table = 'product_lang';
                if ($contentType == 'description') {
                    $ps_element = 'description';
                } else {
                    $ps_element = 'description_short';
                }
                $where = 'id_product = ' . (int) $objId . ' AND id_lang = ' . (int) $idLang;
                break;

            case 'category':
                $ps_table = 'category_lang';
                $ps_element = 'description';
                $where = 'id_category = ' . (int) $objId . ' AND id_lang = ' . (int) $idLang;
                break;

            case 'manufacturer':
                $ps_table = 'manufacturer_lang';
                $ps_element = 'description';
                $where = 'id_manufacturer = ' . (int) $objId . ' AND id_lang = ' . (int) $idLang;
                break;

            case 'supplier':
                $ps_table = 'supplier_lang';
                $ps_element = 'description';
                $where = 'id_supplier = ' . (int) $objId . ' AND id_lang = ' . (int) $idLang;
                break;

            case 'cms':
                $ps_table = 'cms_lang';
                $ps_element = 'content';
                $where = 'id_cms = ' . (int) $objId . ' AND id_lang = ' . (int) $idLang;
                break;
        }
        if ((bool) Configuration::get('EVERSEO_WEBP') === true) {
            ...
            if (strpos($content, '.jpeg') !== false
                || strpos($content, '.jpg') !== false 
                || strpos($content, '.png') !== false 
            ) {
                $content = str_replace('.jpeg', '.jpeg.webp', $content);
                ...
                try {
                    Db::getInstance()->update(
                        $ps_table,
                        [
                            $ps_element => pSQL($content, true),
                        ],
                        $where
                    );
                } catch (Exception $e) {
                    PrestaShopLogger::addLog(
                        'Error on updating webp for ' . $objName . 'and object id ' . $objId
                    );
                }
            }
        } else {
            $content = str_replace('.webp', '', $content);
            $content = str_replace('.webp.webp', '', $content);
            try {
                Db::getInstance()->update(
                    $ps_table,
                    [
